...
# Run a java program with krun, with option --output=raw
...
if [ $# == 1 ];
then
    time timeout 30 krun --parser=aux-kjprep.sh --output=raw -cMainClass="ListItem(\"$MAIN_CLASS\")" -cModelCheck="false" $JAVA_FILE
else
    krun --parser=aux-kjprep.sh --output=raw -cMainClass="ListItem(\"$MAIN_CLASS\")" -cModelCheck="false" --debug $JAVA_FILE
fi
