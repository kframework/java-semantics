...
if [ $# == 4 ]; then
    $TOOLS_DIR/aux-kjsearch-test.sh ${@} $TOOLS_DIR/../model-check/71_strictness/
    exit
fi
...
java -jar $TOOLS_DIR/test-runner.jar \
  -gen $TOOLS_DIR/aux-jdk-run.sh -run $TOOLS_DIR/aux-kjsearch.sh \
  -taskExt java -threads $THREADS -timeout 600 -testsuiteName java-semantics \
