...
if [ $# == 1 ];
then
    time timeout 30 krun --parser="java -jar ../parser/JavaParser.jar" -cMainClass="ListItem(\"$MAIN_CLASS\")" $JAVA_FILE
else
    krun --parser="java -jar ../parser/JavaParser.jar" -cMainClass="ListItem(\"$MAIN_CLASS\")" --debug $JAVA_FILE
fi
